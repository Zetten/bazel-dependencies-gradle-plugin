        ...
        if (createMavenInstallJson) {
            mavenInstallJsonFile.writeText(
                Json(JsonConfiguration.Stable.copy(prettyPrint = true)).stringify(
                    ...
                    MavenInstallJson(dependencies = sortedDependencies, repositories = sortedRepositories)
                )
            )
        }
    }
...
data class MavenInstallJson(
    ...
    constructor(dependencies: List<ProjectDependency>, repositories: List<String>) : this(
        DependencyTree(dependencies, repositories)
    )
}
...
data class DependencyTree(
    ...
    val dependencyTreeSignature: Int = computeDependencyTreeSignature(dependencies)
) {
    constructor(dependencies: List<ProjectDependency>, repositories: List<String>) : this(
        dependencies = computeDependencyTree(dependencies, repositories)
    )
}
